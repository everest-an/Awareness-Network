...
export const appRouter = router({
  ...
  qualityControl: router({
    // Report a vector
    reportVector: protectedProcedure
      .input(z.object({
        vectorId: z.number(),
        reason: z.enum(["spam", "low_quality", "misleading", "copyright", "inappropriate", "other"]),
        description: z.string().optional(),
      }))
      .mutation(async ({ ctx, input }) => {
        const { createVectorReport } = await import("./quality-control");
        return await createVectorReport({
          vectorId: input.vectorId,
    ...
    getVectorReports: protectedProcedure
      .input(z.object({ vectorId: z.number() }))
      .query(async ({ input }) => {
        const { getVectorReports } = await import("./quality-control");
        return await getVectorReports(input.vectorId);
      }),
    ...
    getPendingReports: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const { getPendingReports } = await import("./quality-control");
        return await getPendingReports(input.limit);
      }),
    ...
    resolveReport: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const { resolveReport } = await import("./quality-control");
        await resolveReport({
    ...
    getCreatorReputation: publicProcedure
      .input(z.object({ userId: z.number() }))
      .query(async ({ input }) => {
        const { getCreatorReputation } = await import("./quality-control");
        return await getCreatorReputation(input.userId);
      }),

    // Get quality checks for a vector
    getQualityChecks: publicProcedure
      .input(z.object({ vectorId: z.number() }))
      .query(async ({ input }) => {
        const { getVectorQualityChecks } = await import("./quality-control");
        return await getVectorQualityChecks(input.vectorId);
      }),
  }),
});
